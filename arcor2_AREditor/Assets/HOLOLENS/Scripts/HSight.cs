...
public class HSight : Singleton<HSight>
{
 public GameObject CurrentObject;
        ...
        private void Awake() {
          //  GameManagerH.Instance.OnEditorStateChanged += OnEditorStateChanged;
        }
  ...
  /*     private void OnEditorStateChanged(object sender, EditorStateEventArgs args) {
            switch (args.Data) {
                case GameManagerH.EditorStateEnum.Normal:
                case GameManagerH.EditorStateEnum.InteractionDisabled:
                    enabled = true;
                    break;
                case GameManagerH.EditorStateEnum.Closed:
                    enabled = false;
                    break;
            }
        }
...
/*
        ...
        public IO.Swagger.Model.Pose CreatePoseInTheView(float distance = 0.3f) {
            Ray ray = Camera.main.ScreenPointToRay(new Vector3(Screen.width / 2, Screen.height / 2, 0f));
            Vector3 point = TransformConvertor.UnityToROS(GameManagerH.Instance.Scene.transform.InverseTransformPoint(ray.GetPoint(distance)));
